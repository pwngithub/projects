...
def get_all_sheets(_client, sheet_id):
    """
    Gets a list of all sheets (worksheets) in a Google Sheet document.
    """
    if _client is None:
        return {}
    try:
        ...
        st.error("Spreadsheet not found. Please ensure the SHEET_ID is correct and that you've shared the sheet with the service account email.")
        return {}
    except Exception as e:
        st.error(f"An error occurred while fetching sheet names: {e}")
        return {}
...
def load_data(_worksheet):
    """
    Takes a gspread worksheet object and returns its data as a raw Pandas DataFrame.
    """
    try:
...
def process_data(df):
    ...
    kpi_summary = df_processed.groupby('Type').agg({'Design': 'sum', 'As Built': 'sum'}).reset_index()
...
all_sheets = get_all_sheets(client, SHEET_ID)

if all_sheets:
    # --- Sidebar Controls ---
    st.sidebar.header("Controls & Filters")
    selected_sheet_name = st.sidebar.selectbox("Select Project Sheet:", options=list(all_sheets.keys()))
    selected_worksheet = all_sheets[selected_sheet_name]

    if st.sidebar.button("🔄 Refresh Data"):
        st.cache_data.clear()
        st.rerun()

    raw_dataframe = load_data(selected_worksheet)
    ...
    if raw_dataframe is not None:
        ...
        try:
            ...
            st.error("Could not find the header row. Please ensure a column is named 'Type'.")
            dataframe = None
        ...
        if dataframe is not None:
            ...
            if kpi_data is not None:
                ...
                selected_types = st.sidebar.multiselect(
                    ...
                    options=all_types,
                    default=all_types,
                    key=f"multiselect_{selected_sheet_name}"
                )
                ...
                with tab1:
                    ...
                    if not filtered_kpi_data.empty:
                        ...
                        st.info("No data for selected types.")
                with tab2:
            ...
            with st.expander("🔍 View Raw Data Table"):
                if dataframe is not None:
                    ...
                    st.dataframe(dataframe[columns_to_show].fillna(''))
else:
    st.warning("Could not connect to Google Sheets. Please check your setup and secrets.")
